                    ...
                    {desktopLinks.map((item) =>
                        item.disabled ? (
                            <button
                                ...
                                className="inline-flex cursor-default items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-greenMid/75 transition hover:bg-white/50 hover:text-greenDark"
                            >
                                <item.icon size={16} />
                                {item.label}
                            </button>
                ...
                {open ? (
                    <motion.div initial={{ opacity: 0, y: -18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} transition={{ duration: 0.25 }} className="mx-auto mt-3 max-w-7xl overflow-hidden rounded-[26px] border border-white/70 bg-[rgba(247,250,242,0.95)] p-4 shadow-[0_18px_50px_rgba(52,78,24,0.12)] backdrop-blur-xl lg:hidden">
                        <div className="grid gap-2">
                            {links.map((item) =>
                                item.disabled ? (
                                    <div key={item.label} className="flex items-center justify-between rounded-2xl border border-[#dbe6cf] bg-white px-4 py-3 text-greenMid/80">
                                        <span className="inline-flex items-center gap-3 font-semibold"><item.icon size={16} />{item.label}</span>
                                        <span className="text-[11px] font-bold uppercase tracking-[0.14em] text-greenMid">Soon</span>
                                    </div>
                                ) : item.action ? (
...
function HeroPlantBackdrop({ plants }) {
    ...
    return (
        <div className="absolute inset-0 overflow-hidden">
            ...
            <div className="absolute right-[-6%] top-[8%] hidden w-[62%] rotate-[-6deg] space-y-5 opacity-95 lg:block">
...
export default function LandingPage() {
    ...
    const loadPlants = async () => {
        try {
            ...
            const res = await fetch(`http://localhost:5000/plants?page=${randomPage}&limit=8`);
