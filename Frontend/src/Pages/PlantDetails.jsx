...
export default function PlantDetails() {
    ...
    useEffect(() => {
        ...
        const fetchPlant = async () => {
            try {
                ...
                const res = await fetch(`http://localhost:5000/plants/${id}${q.toString() ? `?${q.toString()}` : ""}`);
    ...
    const defaultVisible = majorPests.length ? majorPests : hostPests.length ? hostPests.slice(0, 6) : pests.slice(0, 6);
    const visiblePests = showAllPests ? pests : defaultVisible;
    const canToggle = pests.length > defaultVisible.length;
    ...
    const hasExtendedDescription = (details.description || "").length > 240;

    const quickFacts = [
        { label: "Type", value: plant.type || details.type },
        { label: "Cycle", value: plant.cycle || details.cycle },
        { label: "Care", value: details.care_level },
        ...
        <div className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.96),_rgba(237,242,229,0.98)_36%,_rgba(214,224,198,1)_100%)] font-dm text-greenDark">
            ...
            <div className="mx-auto max-w-7xl px-4 pb-24 pt-8 md:px-6">
                    ...
                    <div className="mb-8 overflow-hidden rounded-[36px] border border-white/80 bg-white/92 shadow-[0_32px_90px_rgba(52,78,24,0.13)] backdrop-blur-sm">
                        <div className="grid lg:grid-cols-[1fr_1.2fr]">
                            ...
                            <div className="relative overflow-hidden bg-[linear-gradient(135deg,#d5e3be,#b8ce92)]"
                                ...
                                {/* Type badge */}
                                {plant.type ? (
                                    <div className="absolute left-5 top-5 rounded-full border border-white/60 bg-white/80 px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.16em] text-landingPageIcons backdrop-blur-sm">
                                        {plant.type}
                                    </div>
                                ) : null}
                            </div>
                            ...
                            <div className="flex flex-col justify-between p-7 md:p-9 lg:p-10">
                                <div>
                                    ...
                                    <div className="mt-5 flex flex-wrap gap-2">
                                        {plant.cycle ? (
                                            <span className="rounded-full border border-[#dbe6cf] bg-[#f4f8ed] px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.14em] text-landingPageIcons">
                                                {plant.cycle}
                                            </span>
                                        ) : null}
                                        {details.care_level ? (
                                    ...
                                    {/* Description preview */}
                                    {details.description && !hasExtendedDescription ? (
                                        <p className="mt-6 line-clamp-4 text-[0.94rem] leading-[1.88] text-greenMid">
                                            {details.description}
                                        </p>
                                    ) : null}
                                </div>
                ...
                <Reveal className="mb-8">
                    <div className={`overflow-hidden rounded-[30px] border p-7 shadow-[0_18px_52px_rgba(52,78,24,0.08)] md:p-8 ${risk.panel}`}>
                        <div className="flex flex-wrap items-start justify-between gap-5">
                            <div className="flex-1 min-w-0">
                                <div className="mb-4 flex flex-wrap items-center gap-2">
                                    <span className={`inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.16em] ${risk.badge}`}>
                                ...
                                <h2 className="font-playfair text-[clamp(1.5rem,3vw,2.4rem)] font-semibold leading-tight text-greenDark">
                                    {pestRisk?.countryName ? `Pest outlook for ${pestRisk.countryName}` : "Local pest outlook"}
                                </h2>
                                <p className="mt-3 max-w-2xl text-sm leading-7 text-greenMid">
                                    {pestRisk?.summary || "No local pest assessment available yet. Records are sourced from EPPO's global plant protection database."}
                                </p>
                            </div>
                            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[20px] border border-white/70 bg-white/60 shadow-[0_6px_20px_rgba(52,78,24,0.08)] backdrop-blur-sm">
                                <Bug size={26} className="text-greenDark/60" />
                            </div>
                        </div>

                        {pestRisk?.warnings?.length > 0 ? (
                            <div className="mt-5 grid gap-2.5 sm:grid-cols-2">
                                {pestRisk.warnings.slice(0, 4).map((w) => (
                                    <div key={w} className="flex items-start gap-3 rounded-[18px] border border-white/65 bg-white/55 px-4 py-3.5 backdrop-blur-sm">
                                        <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${risk.dot}`} />
                                        <span className="text-sm leading-6 text-greenDark">{w}</span>
                                    </div>
                                ))}
                            </div>
                        ) : null}
                ...
                <div className="grid gap-7 xl:grid-cols-[1.3fr_0.7fr]">
                    ...
                    <div className="space-y-7">
                        ...
                        {/* Full description — shown only if it was clipped above */}
                        {hasExtendedDescription ? (
                            <Reveal>
                                <div className="rounded-[30px] border border-[#dbe6cf] bg-white/90 p-7 shadow-[0_18px_52px_rgba(52,78,24,0.08)] md:p-8">
                                    ...
                                    <p className="text-[0.95rem] leading-[1.9] text-greenMid">{details.description}</p>
                                </div>
                            </Reveal>
                        ) : null}
                                ...
                                {pests.length > 0 ? (
                                    <>
                                        ...
                                        <div className="grid gap-3.5 md:grid-cols-2">
                                            <AnimatePresence>
                                                {visiblePests.map((pest, i) => (
                                                    <motion.article key={pest.eppoCode}
                                                                    initial={{ opacity: 0, y: 12 }}
                                                                    animate={{ opacity: 1, y: 0 }}
                                                                    exit={{ opacity: 0, scale: 0.97 }}
                                                                    transition={{ delay: i * 0.04, duration: 0.45, ease }}
                                                                    className={`overflow-hidden rounded-[22px] border transition-shadow hover:shadow-[0_12px_34px_rgba(52,78,24,0.1)] ${pest.presentInViewerCountry ? "border-[#f5c1ba] bg-[linear-gradient(135deg,#fff9f7,#fff3f0)]" : "border-[#e2ead6] bg-[linear-gradient(135deg,#fafcf7,#f4f9ee)]"}`}>
                                                        <div className="flex items-start gap-3.5 p-4">
                                                            {pest.thumbnailUrl ? (
                                                                <a href={pest.imageUrl || pest.thumbnailUrl} target="_blank" rel="noreferrer" className="shrink-0">
                                                                    <img src={pest.thumbnailUrl} alt={pest.imageCaption || pest.name} loading="lazy"
                                        ...
                                        {canToggle ? (
                                            <div className="mt-5 text-center">
                                                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                                                               onClick={() => setShowAllPests((v) => !v)}
                                                               className="inline-flex items-center gap-2 rounded-full border border-[#dbe6cf] bg-white px-6 py-2.5 text-sm font-semibold text-landingPageIcons transition hover:border-landingPageIcons hover:shadow-[0_8px_24px_rgba(52,78,24,0.1)]">
                                                    {showAllPests ? "Show fewer" : `Show all ${pests.length} pests`}
                                                    <motion.span animate={{ rotate: showAllPests ? 180 : 0 }} transition={{ duration: 0.28 }}>
                                                        <ChevronDown size={14} />
                                                    </motion.span>
                                                </motion.button>
                                            </div>
                    ...
                    <aside className="space-y-7">
                        ...
                        <Reveal>
                            <div className="rounded-[30px] border border-[#dbe6cf] bg-white/90 p-7 shadow-[0_18px_52px_rgba(52,78,24,0.08)]">
                                <Tag icon={Map} label="Taxonomy & origin" />
                                ...
                                {details.care_level ? (
                                    <div className="mt-5 rounded-[18px] border border-[#e2ead6] bg-[#f4f8ed] p-4 text-center">
                                        <p className="mb-2 text-[9.5px] font-bold uppercase tracking-[0.18em] text-greenMid">Care difficulty</p>
                                        <span className={`inline-block rounded-full border px-5 py-1.5 text-sm font-bold ${careLevelColor}`}>
                                            {details.care_level} maintenance
                                        </span>
                                    </div>
                                ) : null}
                            </div>
